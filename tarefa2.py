...
pvel.axis([10e3, 10e9, np.min(phase_vel), np.max(phase_vel)])
...
gvel.axis([10e3, 10e9, np.min(group_vel), np.max(group_vel)])
...
evel.axis([10e3, 10e9, np.min(energy_transf), np.max(energy_transf)])
...
dspr.axis([10e3, 10e9, np.min(pen), np.max(pen)])
plt.show()
